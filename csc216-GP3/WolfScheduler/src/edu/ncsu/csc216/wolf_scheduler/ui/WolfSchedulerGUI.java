...
/**
 * GUI for the WolfScheduler project. The GUI displays the course catalog and
...
public class WolfSchedulerGUI extends JFrame {
	...
	/**
	 * Constructs the WolfSchedulerGUI and sets up the GUI components.
	 */
	...
	private class SchedulerPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnAddCourse) {
				...
				if (row == -1 || row == tableCatalog.getRowCount()) {
					...
					try {
						if (!scheduler.addCourseToSchedule(tableCatalog.getValueAt(row, 0).toString(),
								tableCatalog.getValueAt(row, 1).toString())) {
				...
				try {
					...
					if (eventMeetingDays.length() == 0) {
						JOptionPane.showMessageDialog(WolfSchedulerGUI.this,
								"The event must occur on at least one day.");
						return;
					}
					...
					scheduler.addEventToSchedule(txtEventTitle.getText(), eventMeetingDays, eventStartTime,
							eventEndTime, txtDetails.getText());
		...
		private class CourseTableModel extends AbstractTableModel {
			 ...
			 * 
			 * @param catalog flag to determine if updating the catalog (true) or schedule
			 *                (false)
			 */
	...
	private class SchedulePanel extends JPanel implements ActionListener {
		...
		/**
		 * {@link FullCourseTableModel} is the object underlying the {@link JTable}
		 * object that displays the list of {@link Course}s, and all their data, to the
		 * user.
		 * 
		 * @author Sarah Heckman
		 */
		private class FullCourseTableModel extends AbstractTableModel {
			...
			private static final long serialVersionUID = 1L;
			/** Column names for the table */
			private String[] columnNames = { "Name", "Section", "Title", "Credits", "Instructor", "Meeting Information",
					"Details" };
			/** Data stored in the table */
			private Object[][] data;
