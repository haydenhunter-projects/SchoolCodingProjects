...
/**
 * Container for the WolfTickets project that provides the user the ability to
 * interact with groups, ticket lists, and tickets.
 *
...
public class WolfTicketsGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a WolfTicketsGUI object that will contain a JMenuBar and a JPanel
	 * that will hold different possible views of the data in the ServiceWolf.
	 */
	...
	/**
	 * Makes the GUI Menu bar that contains options working with a file containing
	 * service groups and incidents or for quitting the application.
	 */
		...
		if (e.getSource() == itemNew) {
			if (group != null && group.isChanged()) {
				int select = JOptionPane.showConfirmDialog(null,
						"Current Group is unsaved. Would you like to save before creating a new Group?", "Warning",
	...
	private class CategoryPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == comboCategorys) {
				...
				try {
					String categoryName = JOptionPane.showInputDialog(this, "Category Name?", "Create New Category",
				...
				try {
					if (group.getCurrentCategory() instanceof ActiveTicketList) {
						JOptionPane.showMessageDialog(WolfTicketsGUI.this,
								"The Active Tickets list may not be edited.");
					} else {
						String categoryName = (String) JOptionPane.showInputDialog(this, "Update Category Name Name?",
								"Edit Category", JOptionPane.QUESTION_MESSAGE, null, null,
								group.getCurrentCategory().getCategoryName());
				...
				try {
					if (group.getCurrentCategory() instanceof ActiveTicketList) {
						JOptionPane.showMessageDialog(WolfTicketsGUI.this,
								"The Active Tickets list may not be deleted.");
					} else {
						int selection = JOptionPane.showOptionDialog(this,
								"Are you sure you want to delete " + group.getCurrentCategory().getCategoryName() + "?",
								"Delete Category", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null,
								null);
						if (selection == 0) { // Yes
		...
		public void updateCategorys() {
			if (group == null) {
				...
				for (String categoryName2 : categoryNames) {
					comboCategorys.addItem(categoryName2);
				}
		...
		/**
		 * TicketTableModel is the object underlying the JTable object that displays the
		 * list of Tickets to the user.
		 */
		private class TicketTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the IncidentTableModel by requesting the latest information from
			 * the IncidentTableModel.
			 */
	...
	private class TicketPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAddEdit) {
				try {
					if (idx == -1) {
						Ticket t = new Ticket(txtTicketName.getText(), txtDescription.getText(),
								checkActive.isSelected());
						group.addTicket(t);
					} else {
						group.editTicket(idx, txtTicketName.getText(), txtDescription.getText(),
								checkActive.isSelected());
					}
					setTicket(-1);
				} catch (IllegalArgumentException iae) {
