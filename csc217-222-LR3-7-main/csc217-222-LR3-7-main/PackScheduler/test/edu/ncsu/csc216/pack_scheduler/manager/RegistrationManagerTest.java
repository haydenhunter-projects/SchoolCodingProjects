...
public class RegistrationManagerTest {
	...
	public void setUp() throws Exception {
		...
		manager.getFacultyDirectory().loadFacultyFromFile("test-files/faculty_records_test.txt");
	}	
	/**
	 * Test class for GetCourseCatalog
	 */
	@Test
	public void testGetCourseCatalog() {
		RegistrationManager r1 = RegistrationManager.getInstance();
		assertEquals(0, r1.getCourseCatalog().getCourseCatalog().length);
		r1.getCourseCatalog().addCourseToCatalog("Csc216", "Software Development", "001", 4, "sesmith", 150, "MW", 100, 200);
		assertEquals(1, r1.getCourseCatalog().getCourseCatalog().length);
	}

	/**
	 * Test class for GetStudentDirectory
	 */
	@Test
	public void testGetStudentDirectory() {
		RegistrationManager r1 = RegistrationManager.getInstance();
		assertEquals(0, r1.getStudentDirectory().getStudentDirectory().length);
		r1.getStudentDirectory().addStudent("Travis", "Koekemoer", "001", "takoekem@ncsu.edu", "Nice", "Nice", 3);
		assertEquals(1, r1.getStudentDirectory().getStudentDirectory().length);
	}
	...
	public void testLogin() {
		RegistrationManager r1 = RegistrationManager.getInstance();
//		r1.logout();
//		assertTrue(r1.login("flast", "password"));
//		r1.logout();
//		assertFalse(r1.login("flast", "wrong"));
//		Exception e3 = assertThrows(IllegalArgumentException.class, 
//				() -> r1.login("wrong", "password"));
//		assertEquals("User doesn't exist.", e3.getMessage());
		...
		assertTrue(r1.login("001", "Nice"));
		assertFalse(r1.login("flast", "password"));
		r1.logout();
		assertFalse(r1.login("001", "wrong"));
	...
	public void testLogout() {
		...
		r1.getStudentDirectory().addStudent("Travis", "Koekemoer", "001", "takoekem@ncsu.edu", "Nice", "Nice", 3);
		r1.login("001", "Nice");
		User u1 = r1.getCurrentUser();
		r1.logout();
		assertNotEquals(u1, r1.getCurrentUser());
		assertEquals(null, r1.getCurrentUser());
	}

	/**
	 * Test class for GetCurrentUser
	 */
	@Test
	public void testGetCurrentUser() {
		...
		r1.login("001", "Nice");
		User u1 = r1.getCurrentUser();
		assertEquals(u1, r1.getCurrentUser());
		assertEquals("Travis", r1.getCurrentUser().getFirstName());
		assertEquals("Koekemoer", r1.getCurrentUser().getLastName());
		assertEquals("001", r1.getCurrentUser().getId());
		assertEquals("takoekem@ncsu.edu", r1.getCurrentUser().getEmail());
	}
	...
	/**
	 * Test for faculty operations in RegistrationManager
	 */
	@Test
	public void testFaculty() {
		Faculty f = new Faculty("fname", "lname", "flname", "name@ncsu.edu", "pw", 3);
		Course c = new Course("CSC216", "Software Development Fundamentals", "001", 3, null, 150, "MW");
		Course c2 = new Course("CSC217", "Software Development Fundamentals Lab", "001", 3, null, 150, "TH");
		...
		manager.login(registrarUsername, registrarPassword);
		manager.getFacultyDirectory().addFaculty("fname", "lname", "flname", "name@ncsu.edu", "pw", "pw", 3);
		manager.addFacultyToCourse(c, f);
		String[][] toCompare =  f.getSchedule().getScheduledCourses();
		assertEquals("CSC216", toCompare[0][0]);
		assertEquals(1, f.getSchedule().getScheduledCourses().length);
		
		manager.addFacultyToCourse(c2, f);
		toCompare =  f.getSchedule().getScheduledCourses();
		assertEquals(2, f.getSchedule().getScheduledCourses().length);
		assertEquals("CSC217", toCompare[1][0]);
		
		manager.removeFacultyFromCourse(c, f);
		toCompare =  f.getSchedule().getScheduledCourses();
		assertEquals(1, f.getSchedule().getScheduledCourses().length);
		assertEquals("CSC217", toCompare[0][0]);
		
		manager.resetFacultySchedule(f);
		assertEquals(0, f.getSchedule().getScheduledCourses().length);
